...
def get_egfr(patient_id):
    ...
    if row is not None:
        if row[2] is None:
            return f"Poslední měření eGFR je neúplné {row[3]}"
        if row[4] != "ml/s/1,73 m2":
            return "Jednotka eGFR není ml/s/1,73 m2"
...
def calculate_egfr(patient_id):
    try:
        ...
        if patient_data is None:
            return "Nelze spočítat bez dat o pacientovi"
        dob = datetime.datetime.strptime(patient_data[0], '%Y-%m-%d')
        sex = patient_data[1]
        if row[2] is None:
            return f"Poslední měření kreatininu je neúplné {row[3]}"
        if row[4] != 'µmol/l':
            return "Jednotka kreatininu není µmol/l"
